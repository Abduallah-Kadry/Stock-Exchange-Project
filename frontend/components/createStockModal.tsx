...
export function CreateStockModal({onStockCreated}: CreateStockModalProps) {
	...
	const validateForm = () => {
		...
		if (!formData.description.trim()) {
			...
			newErrors.description = "Description must be between 1 and 30 characters"
		}

		// Price validation
		if (!formData.currentPrice) {
			newErrors.currentPrice = "Price is mandatory"
		} else {
			const price = parseFloat(formData.currentPrice)
			if (isNaN(price)) {
				newErrors.currentPrice = "Price must be a valid number"
			} else if (price < 0) {
				newErrors.currentPrice = "Price must be equal to or greater than zero"
			}
		}
